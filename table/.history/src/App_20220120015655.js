...
const Cpn = (props) => {
  ...
  return (
    <>
      ...
      <Modal visible={isModalVisible} onOk={handleOk} onCancel={handleCancel} width='630px'  closable='false' footer={null}>
...
export default memo(function App() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width:'150px',
      render: (item,idx) => (
        <Cpn key={Math.random()*100} item={item}></Cpn>
      ),
    },
  ];
  return (
  <div style={{width:'1120px',margin:'auto'}}>
    <Table columns={columns} dataSource={datas} bordered='true' scroll={{ y: '550px' }} pagination={false}/>
  </div>
  );
});
