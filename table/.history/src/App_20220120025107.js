...
export default memo(function App() {
  ...
  const filterData = datas.map(item=>{
    return item.tags.map(it=>({text:it,value:it}))
  }).flat()
  console.log('filterData',filterData);
  console.log('datas:',datas);

  function search(value){
    setdatas(datas.filter(item=>(item.tags.some(it=>it.indexOf(value)>-1))))
  }
  ...
  <div style={{width:'1120px',margin:'auto'}}>
    ...
    <Table columns={columns} dataSource={datas} bordered='true' scroll={{ y: '530px' }} pagination={false}/>
  </div>
  );
});
